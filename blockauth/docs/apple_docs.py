...
apple_native_verify_schema = {
    ...
    "description": (
        ...
        "to allowlist the iOS app's bundle identifier(s). When `authorization_code` is supplied, "
        "BlockAuth redeems it with `client_id=<id_token aud>` and a client_secret whose `sub` matches that client_id.\n"
        "\n"
        "**Error Handling:**\n"
