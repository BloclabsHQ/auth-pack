        ...
        if method == "email":
            try:
                ...
                raise ValidationError(
                    detail={
                        "identifier": "Invalid email address format. Please provide a valid email (e.g., user@example.com)."
                    },
                    code=4001,
                )
        elif method == "sms":
            if not is_valid_phone_number(identifier):
                raise ValidationError(
                    detail={
                        "identifier": "Invalid phone number format. Please provide a valid phone number with country code (e.g., +1234567890)."
                    },
                    code=4001,
                )
    ...
    def validate(self, data):
        ...
        try:
            ...
            if not is_valid_phone_number(identifier):
                raise ValidationError(
                    detail={
                        "identifier": "Invalid identifier format. Please provide a valid email address or phone number."
                    },
                    code=4001,
                )
            data["phone_number"] = identifier
