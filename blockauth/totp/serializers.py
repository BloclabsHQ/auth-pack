...
from .constants import DEFAULTS, TOTPConfigKeys, TOTPStatus
...
class TOTPSetupRequestSerializer(serializers.Serializer):
    ...
    issuer = serializers.CharField(
        max_length=100,
        required=False,
        help_text="Custom issuer name for authenticator app"
    )
...
class TOTPSetupResponseSerializer(serializers.Serializer):
    ...
    secret = serializers.CharField(
        help_text="Base32-encoded TOTP secret (show to user for manual entry)"
    )
    ...
    backup_codes = serializers.ListField(
        ...
        help_text="One-time backup codes (show once, then discard)"
    )
    qr_code_data = serializers.CharField(
        required=False,
        help_text="Base64-encoded QR code image (if generated server-side)"
    )
...
class TOTPVerifyRequestSerializer(serializers.Serializer):
    ...
    code = serializers.CharField(
        min_length=6,
        max_length=16,  # Allow for backup codes
        help_text="TOTP code or backup code"
    )
...
class TOTPVerifyResponseSerializer(serializers.Serializer):
    ...
    verification_type = serializers.ChoiceField(
        choices=['totp', 'backup'],
        help_text="Type of code that was verified"
    )
...
class TOTPStatusResponseSerializer(serializers.Serializer):
    ...
    enabled = serializers.BooleanField(
        help_text="Whether TOTP is enabled"
    )
    status = serializers.ChoiceField(
        choices=[s.value for s in TOTPStatus],
    ...
    enabled_at = serializers.DateTimeField(
        required=False,
        allow_null=True,
        help_text="When TOTP was enabled"
    )
...
class TOTPDisableRequestSerializer(serializers.Serializer):
    ...
    code = serializers.CharField(
        ...
        max_length=16,
        required=False,
        help_text="TOTP code or backup code for verification (recommended)"
    )
    password = serializers.CharField(
        required=False,
        help_text="User password as alternative verification"
    )
    ...
    def validate(self, attrs):
        ...
        if not attrs.get('code') and not attrs.get('password'):
            raise serializers.ValidationError(
                "Either 'code' or 'password' must be provided for verification"
            )
        return attrs
...
class BackupCodesResponseSerializer(serializers.Serializer):
    ...
    backup_codes = serializers.ListField(
        child=serializers.CharField(),
        help_text="New backup codes (show once, then discard)"
    )
    count = serializers.IntegerField(
        help_text="Number of backup codes generated"
    )
...
class TOTPErrorSerializer(serializers.Serializer):
    ...
    lockout_remaining_seconds = serializers.IntegerField(
        required=False,
        help_text="Seconds remaining on account lockout (if locked)"
    )
