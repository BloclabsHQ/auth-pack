...
SENSITIVE_PARAMS = frozenset([
    ...
    'code', 'otp', 'pin', 'backup_code', 'encrypted', 'signature'
])
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs) -> Any:
            ...
            blockauth_logger.info(
                f"AUDIT: {func_event}",
                sanitize_log_context(audit_data)
            )
            ...
            try:
                ...
                if log_result and result is not None:
                    if isinstance(result, dict):
                        ...
                        success_data['result'] = {
                            k: '[REDACTED]' if _is_sensitive(k) else _sanitize_value(v)
                ...
                blockauth_logger.success(
                    f"AUDIT: {func_event}.success",
                    sanitize_log_context(success_data)
                )
                ...
                if severity in ('ERROR', 'CRITICAL'):
                    blockauth_logger.error(
                        f"AUDIT: {func_event}.failed",
                        sanitize_log_context(failure_data)
                    )
                else:
