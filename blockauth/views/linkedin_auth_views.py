...
from blockauth.docs.social_auth_docs import linkedin_auth_login_schema, linkedin_auth_callback_schema
...
class LinkedInAuthCallbackView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            blockauth_logger.error(
                "LinkedIn login validation error",
                {"error": str(ve), "data": sanitize_log_context(request.data)}
            )
            raise APIException()
        except Exception as e:
            blockauth_logger.error(
                "LinkedIn login unexpected error",
